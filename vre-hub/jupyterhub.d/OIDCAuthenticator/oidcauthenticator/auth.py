...
class OIDCAuthenticator(GenericOAuthenticator):
    ...
    verify_aud = Bool(
        default_value=False,
        config=True,
        help="If audience claim should be verified"
    )
    ...
    allowed_audience = List(
        ...
        default_value=[],
        config=True,
        help="List of allowed audience claim"
    )
            ...
            with request.urlopen('%s/.well-known/openid-configuration' % self.oidc_issuer) as response:
                ...
                if self.config.check_signature:
                    ...
                    with request.urlopen(jwks_uri) as jkws_response:
                        ...
                        self.public_key = RSAAlgorithm(
                            RSAAlgorithm.SHA256).from_jwk(jwk_data['keys'][0])
    ...
    def _decode_token(self, token, options={}):
        ...
        try:
            decoded_token = jwt.decode(token, self.public_key, options=options,
                                       audience=self.allowed_audience, issuer=self.oidc_issuer, algorithms=self.jwt_signing_algorithms)
    ...
    async def authenticate(self, handler, data=None):
        ...
        try:
            ...
            self.log.error(
                "Unable to retrieve the roles, denying access.", exc_info=True)
            return None
        ...
        if not self._validate_roles(user_roles):
            self.log.info(
                f"User '{user['name']}' doesn't have apropriate role to be allowed")
            return None
    ...
    async def refresh_user(self, user, handler=None):
        ...
        try:
            ...
            decoded_access_token = self._decode_token(
                auth_state['access_token'])
            # no verification of the refresh token signature as it is not needed, the auth server
            # verifies it
            decoded_refresh_token = self._decode_token(
                auth_state['refresh_token'], options={"verify_signature": False, "verify_iss": False, "verify_aud": False})
            ...
            diff_refresh = (
                decoded_refresh_token['exp'] - time.time()) if 'exp' in decoded_refresh_token else 0
            ...
            if diff_access > self.auth_refresh_age:
                ...
                access_token, refresh_token = self._refresh_token(
            ...
            self.log.error(
                "Failure calling the renew endpoint: %s (code: %s)" % (e.read(), e.code))
